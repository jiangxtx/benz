...
比如`emit`方法的传参，就采用了显示接受`a1`~`a5`等 5 个用户传参，然后不遗余力的通过`switch`逻辑进行一一处理。这么做，乍一看倒显得很冗余，且有失代码的雅观。但是，细细品来，容我斗胆猜测：**应该是追求极致性能的结果吧**，毕竟通常使用看来，用户顶多也就传2~3个参数了。这5个传参场景，足矣覆盖了`95%`以上的情形。
